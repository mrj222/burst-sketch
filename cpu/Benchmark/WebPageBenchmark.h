#ifndef _WEBBENCHMARK_H_
#define _WEBBENCHMARK_H_
...
class WebBenchmark {
public:
	WebBenchmark(WebDataset* _web): web(_web) {}
	~WebBenchmark() {}
	void Compare(std::vector<Burst<uint64_t>> &result, std::vector<Burst<uint64_t>> &ground_truth) {
        ...
        // return 200 * (double)(correct + overlap) / result.size() * (double)(correct + overlap) / ground_truth.size() / ((double)(correct + overlap) / result.size() + (double)(correct + overlap) / ground_truth.size());
    ...
    void Run(int memory, int flag) {
        ...
        if (!count_based)
            start_time = web->dataset[0].timestamp;
        ...
        // std::cout << web->dataset[0].timestamp << std::endl;
        // std::cout << web->dataset[run_length - 1].timestamp << std::endl;
        for (uint32_t i = 0; i < run_length; ++i) {
            if (count_based)
                correct_detector->insert(web->dataset[i].id, i);
            else 
                correct_detector->insert(web->dataset[i].id, web->dataset[i].timestamp);
        }
        ...
        if (flag & FLAG_BURST) {
            ...
            for (uint32_t i = 0; i < run_length; ++i) {
                if (count_based)
                    burst_sketch->insert(web->dataset[i].id, i);
                else 
                    burst_sketch->insert(web->dataset[i].id, web->dataset[i].timestamp);
            }
        ...
        if (flag & FLAG_DYNAMIC) {
            ...
            for (uint32_t i = 0; i < run_length; ++i) {
                if (count_based)
                    dynamic_sketch->insert(web->dataset[i].id, i);
                else 
                    dynamic_sketch->insert(web->dataset[i].id, web->dataset[i].timestamp);
            }
        ...
        if (flag & FLAG_CM) {
            ...
            for (uint32_t i = 0; i < run_length; ++i) {
                if (count_based)
                    cm_sketch->insert(web->dataset[i].id, i);
                else 
                    cm_sketch->insert(web->dataset[i].id, web->dataset[i].timestamp);
            }
        ...
        if (flag & FLAG_TOPIC) {
            ...
            for (uint32_t i = 0; i < run_length; ++i) {
                if (count_based)
                    topic_sketch->insert(web->dataset[i].id, i);
                else 
                    topic_sketch->insert(web->dataset[i].id, web->dataset[i].timestamp);
            }
        ...
        if (flag & FLAG_PBE1) {
		    ...
		    for (uint32_t i = 0; i < run_length; ++i) {
                cm_pbe_1->insert(web->dataset[i].id, i);
            }
        ...
        if (flag & FLAG_PBE2) {
		    ...
		    for (uint32_t i = 0; i < run_length; ++i) {
                cm_pbe_2->insert(web->dataset[i].id, i);
            }
